...
I planned and implemented the following three extra-credit features:

- **Metadata Filtering:** Restrict retrieval results to a specific source document.
- **Conversational Memory:** Use recent questions as context for follow-up questions.
- **Second Embedding Model:** Compare retrieval results between two embedding models.
...
I installed Sentence Transformers 3.4.1 and temporarily configured my application to use the alternative embedding model. I created a separate ChromaDB index using the `minilm_l12` variant to preserve the original index.
